...
export const useContactForm = function () {
  ...
  const formik = useFormik({
    ...
    validate: (values) => {
      ...
      } else if (values.companyName.split(' ').length < 1) {
        errors.companyName = 'company Name must be at least 1 word';
      }
      ...
      } else if (values.comment.split(' ').length < 1) {
        errors.comment = 'comment must be at least 1 word';
      }
